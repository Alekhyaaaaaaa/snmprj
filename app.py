...
mydb=connection.MySQLConnection(user='root',password='akhynaidu',host='localhost',db='snmprjdb')

app = Flask(__name__)
app.config['SESSION_TYPE']='filesystem' # data stored in file format
Session(app)
excel.init_excel(app)
app.secret_key='Akhi0703'
...
def viewfile(fid):
    if session.get('user'):
        cursor=mydb.cursor(buffered=True)
        cursor.execute('select fid,file_name,file_data from filedata where fid=%s and added_by=%s',
                    [fid,session.get('user')])
        fdata=cursor.fetchone()
        data=BytesIO(fdata[2]) 
        mime_type, _=guess_type(fdata[1])
        print(mime_type)
        return send_file(data,download_name=fdata[1],mimetype=mime_type
                        or 'application/octat-stream',as_attachment=False)  
    else:
        flash('Please Login First')
        return redirect(url_for('login')) 
...
def downloadfile(fid):
    if session.get('user'):
        ...
        fdata=cursor.fetchone()
        data=BytesIO(fdata[2]) 
        flash(f'{fdata[1]} downloaded successfully')
        return send_file(data,download_name=fdata[1],as_attachment=True)  
    else:
        flash('Please Login First')
        return redirect(url_for('login'))
...
def getexceldata():
    if session.get('user'):
        cursor=mydb.cursor(buffered=True)
        cursor.execute('select *from notes where added_by=%s',
                       [session.get('user')])
